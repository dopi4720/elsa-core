...
export class ElsaWorkflowInstanceListScreen {
  ...
  connectedCallback() {
    if (!!this.history) this.unlistenRouteChanged = this.history.listen(e => this.routeChanged(e));
  }

  disconnectedCallback() {
    if (!!this.unlistenRouteChanged) this.unlistenRouteChanged();
  }
  ...
  async routeChanged(e: LocationSegments) {
    if (!e.pathname.toLowerCase().endsWith('workflow-instances')) return;
  ...
  render() {
    ...
    const renderRetryIcon = function () {
      return (
        <svg
          class="elsa-h-5 w-5 elsa-text-gray-500"
          width="24"
          height="24"
          viewBox="0 0 24 24"
          stroke-width="2"
    ...
    return (
      <div>
        <div class="elsa-relative elsa-z-10 elsa-flex-shrink-0 elsa-flex elsa-h-16 elsa-border-b elsa-border-gray-600">
        ...
        <div class="elsa-p-8 elsa-flex elsa-content-end elsa-justify-right elsa-bg-gray-700 elsa-space-x-4">
          <div class="elsa-flex-shrink-0">{this.renderBulkActions()}</div>
          <div class="elsa-flex-1">&nbsp;</div>
  ...
  renderBulkActions() {
    const bulkActionIcon = (
      <svg class="elsa-mr-3 elsa-h-5 elsa-w-5 elsa-text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  renderWorkflowFilter() {
    ...
    let items: Array<DropdownButtonItem> = latestWorkflowBlueprints.map(x => {
      ...
      if (!!history) item.url = this.buildFilterUrl(x.id, selectedWorkflowStatus, selectedOrderBy, null, selectedCorrelationId);
    ...
    if (!!history) allItem.url = this.buildFilterUrl(null, selectedWorkflowStatus, selectedOrderBy, null, selectedCorrelationId);
  ...
  renderStatusFilter() {
    ...
    const items: Array<DropdownButtonItem> = statuses.map(x => {
      ...
      if (!!history) item.url = this.buildFilterUrl(this.selectedWorkflowId, x, this.selectedOrderByState, null, selectedCorrelationId);
    ...
    return (
      <elsa-dropdown-button
        text={selectedWorkflowStatusText}
        items={items}
        icon={renderIcon()}
        origin={DropdownButtonOrigin.TopRight}
  ...
  renderPageSizeFilter() {
    ...
    const items: Array<DropdownButtonItem> = pageSizes.map(x => {
      ...
      if (!!history) item.url = this.buildFilterUrl(this.selectedWorkflowId, this.selectedWorkflowStatus, this.selectedOrderByState, x, selectedCorrelationId);
    ...
    const renderIcon = function () {
      return (
        <svg
          class="elsa-h-5 elsa-w-5 elsa-text-gray-400 elsa-mr-2"
          width="24"
          height="24"
          viewBox="0 0 24 24"
          stroke-width="2"
  ...
  renderOrderByFilter() {
    ...
    const items: Array<DropdownButtonItem> = orderByValues.map(x => {
      ...
      if (!!history) item.url = this.buildFilterUrl(this.selectedWorkflowId, this.selectedWorkflowStatus, x, null, selectedCorrelationId);
