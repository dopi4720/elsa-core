  ...
  render() {
    ...
    const renderChevronLeft = function () {
      ...
      return (
        <a href={`${getNavUrl(page - 1)}`}
           onClick={e => self.onNavigateClick(e, page - 1)}
           class="elsa-relative elsa-inline-flex elsa-items-center elsa-px-2 elsa-py-2 elsa-rounded-l-md elsa-border elsa-border-gray-400 elsa-bg-gray-500 elsa-text-sm elsa-leading-5 elsa-font-medium elsa-text-gray-400 hover:elsa-text-gray-400 focus:elsa-z-10 focus:elsa-outline-none focus:elsa-border-blue-300 focus:elsa-shadow-outline-blue active:elsa-bg-gray-100 active:elsa-text-gray-500 elsa-transition elsa-ease-in-out elsa-duration-150"
           aria-label="Previous">
    ...
    const renderChevronRight = function () {
      ...
      return (
        <a href={`${getNavUrl(page + 1)}`}
           onClick={e => self.onNavigateClick(e, page + 1)}
           class="-elsa-ml-px elsa-relative elsa-inline-flex elsa-items-center elsa-px-2 elsa-py-2 elsa-rounded-r-md elsa-border elsa-border-gray-500 elsa-bg-gray-600 elsa-text-sm elsa-leading-5 elsa-font-medium elsa-text-gray-400 hover:elsa-text-gray-400 focus:elsa-z-10 focus:elsa-outline-none focus:elsa-border-blue-300 focus:elsa-shadow-outline-blue active:elsa-bg-gray-100 active:elsa-text-gray-500 elsa-transition elsa-ease-in-out elsa-duration-150"
           aria-label="Next">
    ...
    const renderPagerButtons = function () {
      ...
      for (let i = fromPage; i < toPage; i++) {
        ...
        if (isCurrent) {
          buttons.push(<span
            class={`-elsa-ml-px elsa-relative elsa-inline-flex elsa-items-center elsa-px-4 elsa-py-2 elsa-border elsa-border-gray-500 elsa-text-sm elsa-leading-5 elsa-font-medium elsa-bg-blue-600 elsa-text-white ${leftRoundedClass} ${rightRoundedClass}`}>
                        {i + 1}
                    </span>);
        } else {
          buttons.push(<a href={`${getNavUrl(i)}`}
                          onClick={e => self.onNavigateClick(e, i)}
                          class={`-elsa-ml-px elsa-relative elsa-inline-flex elsa-items-center elsa-px-4 elsa-py-2 elsa-border elsa-border-gray-500 elsa-bg-gray-600 elsa-text-sm elsa-leading-5 elsa-font-medium elsa-text-gray-400 hover:elsa-text-gray-500 focus:elsa-z-10 focus:elsa-outline-none active:elsa-bg-gray-100 active:elsa-text-gray-300 elsa-transition elsa-ease-in-out elsa-duration-150 ${leftRoundedClass}`}>
            {i + 1}
          </a>)
        }
      }
    ...
    return (
      <div class="elsa-bg-gray-700 elsa-px-4 elsa-py-3 elsa-flex elsa-items-center elsa-justify-between elsa-border-t elsa-border-gray-600 sm:elsa-px-6">
        ...
        <div class="hidden sm:elsa-flex-1 sm:elsa-flex sm:elsa-items-center sm:elsa-justify-between">
          <div>
            <p class="elsa-text-sm elsa-leading-5 elsa-text-gray-400 elsa-space-x-0-5">
              <span>{t('From')}</span>
